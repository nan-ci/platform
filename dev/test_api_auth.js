...
o['GET /link/discord with a session generate a state without a speciality'] = {
  it: async () => {
    ...
    eq(NAN.entries[`discord:${state}`], {
      ...
      value: '',
      key: `discord:${state}`,
      metadata: { name: session, user, speciality: undefined },
    })
...
o['GET /link/discord with a session generate a state'] = {
  it: async () => {
    ...
    await db.set(
      session,
      whoTest === 'student' ? { ...user, role: 'visitor' } : user,
    )
    const { body, options } = await GET(
      `/link/discord${whoTest === 'student' ? '?speciality=javascript' : ''}`,
      {
        headers: { cookie: `nan-session=${session}` },
      },
    )
    ...
    eq(NAN.entries[`discord:${state}`], {
      ...
      metadata: {
        name: session,
        user: whoTest === 'student' ? { ...user, role: 'visitor' } : user,
        speciality: whoTest === 'student' ? speciality : undefined,
      },
    })
...
o['GET /auth/discord  with a proper state '] = {
  it: async () => {
    ...
    const metadata =
      whoTest === 'professor'
        ? { user, name }
        : { user: { ...user, role: 'visitor', speciality } }
    await NAN.put('discord:exists-state', '', { metadata })
    ...
    const discordUser =
      whoTest === 'professor'
        ? { ...user, discordId, email, avatar, speciality, role: 'professor' }
        : { ...user, role, discordId, email, avatar }
    eq(
      headers.location,
      `/${
        whoTest === 'professor' ? 'professor/dashboard' : 'learningchoice'
      }?${new URLSearchParams(discordUser)}`,
    )
