...
export const getQuiz = () => JSON.parse(localStorage.getItem('quiz'))
export const getUser = () => JSON.parse(localStorage.getItem('user'))

export const time = (type) => {
  switch (type) {
    case 'all':
      return moment.duration(moment(getQuiz().end_date).diff(moment()))
    case 'asSeconds':
      return moment
        .duration(moment(getQuiz().end_date).diff(moment()))
        .asSeconds()
  }
}
