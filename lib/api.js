import { API, CI } from './env.js'
import { useState, useEffect } from 'preact/hooks'

const addDevCookie = CI
  ? (opts) => ({
      ...opts,
      headers: {
        ...opts?.headers,
        'x-nan-cookie': JSON.parse(localStorage.user)['nan-session'],
      },
    })
  : (opts) => opts
...
const get = (_, method) => {
  const fetcher = async (path, { params, ...opts } = {}) => {
    ...
    const fetchOpts = { method, credentials: 'include', ...addDevCookie(opts) }
    const res = await fetch(`${API}/${path}${strParams}`, fetchOpts)
    console.log(path, '->', fetchOpts)
